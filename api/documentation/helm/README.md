# medewerkercatalogus chart

The Helm chart installs medewerkercatalogus and by default the following dependencies using subcharts:
...
```bash
helm repo add medewerkercatalogus https://raw.githubusercontent.com/ConductionNL/medewerkercatalogus/master/api/helm/
helm repo update
```
...
```bash
helm install my-medewerkercatalogus ConductionNL/medewerkercatalogus --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `medewerkercatalogus` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `medewerkercatalogus` |
